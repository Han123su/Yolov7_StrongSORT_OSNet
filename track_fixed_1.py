                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        try:
                            ...
                            # contour area
                            curve, len_straight = curve_rate(skeleton_idx)
                            ...
                            ########################### If want to draw the image (including save) ################################                 
                            if not os.path.isdir(save_dir / 'process_img'):
                                os.makedirs(save_dir / 'process_img', exist_ok=True) 
                            
                            color_image = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
                            new_skeleton_idx=new_skeleton_idx[:, [1, 0]]
                            
                            if np.array_equal(new_skeleton_idx[0], detect_pts[0]):  # Confirm new_skeleton_idx sorting
                                new_skeleton_idx = new_skeleton_idx
                            else:                                                   # If not from beginning to end
                                new_skeleton_idx = np.flipud(new_skeleton_idx)      # reverse order

                            for pt1, pt2 in zip(new_skeleton_idx, new_skeleton_idx[1:]):
                                pt1 = tuple(pt1.astype(np.int32))
                                pt2 = tuple(pt2.astype(np.int32))
                                cv2.line(color_image, pt1, pt2, (255, 0, 0), 1, cv2.LINE_AA)
                            
                            for pt in new_skeleton_idx.astype(np.int32):
                                cv2.circle(color_image, tuple(pt), 2, (255, 0, 0), -1)

#                            for idx, pt in enumerate(new_skeleton_idx.astype(np.int32)):
#                                if idx < 2:
#                                    cv2.circle(color_image, tuple(pt), 2, (0, 0, 255), -1)
#                                else:
#                                    cv2.circle(color_image, tuple(pt), 2, (255, 0, 0), -1)
                                
                            for idx, pt in enumerate(detect_pts.astype(np.int32)):
                                if idx < 2:
                                    cv2.circle(color_image, tuple(pt), 2, (0, 0, 255), -1)
                                else:
                                    cv2.circle(color_image, tuple(pt), 2, (0, 255, 0), -1)

                            if z_img is not None:
                                try:
                                    cv2.imwrite(str(save_dir / 'process_img' / f'{frame_idx}.jpg'), color_image)
                            ...
                            ############################### Feature point rotation alignment ####################################
                            if not os.path.isdir(save_dir / 'alignment'):
                                os.makedirs(save_dir / 'alignment', exist_ok=True) 
                            ...
                            if frame_idx == 2:
                                img1 = color_image
                                #point1 = detect_pts[:2]
                                point1 = new_skeleton_idx[:2]
                            else:
                                img2 = color_image
                                #point2 = detect_pts[:2]
                                point2 = new_skeleton_idx[:2]
                                aligned_img = align_images(img1, img2, point1, point2)
                                
                                cv2.imwrite(str(save_dir / 'alignment' / f'{frame_idx}.jpg'), aligned_img)
                            ########################################## save over ################################################
